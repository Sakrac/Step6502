...
//	memcpy(ram, aStartupRAM, 64*1024);

	wchar_t exePath[MAX_PATH];
	if (int exePathLen = GetModuleFileName(GetModuleHandleW(NULL), exePath, MAX_PATH)) {
		while (exePathLen && exePath[exePathLen-1] != L'\\' && exePath[exePathLen-1] != L'/')
			exePathLen--;
		wcscpy_s(exePath+exePathLen, MAX_PATH-exePathLen, L"bootram.bin");
		FILE *f;
		if (_wfopen_s(&f, exePath, L"rb") == 0) {
			fseek(f, 0, SEEK_END);
			uint32_t size = ftell(f);
			fseek(f, 0, SEEK_SET);
			if (size > 0x10000) {
				fseek(f, 0x10000-size, SEEK_SET);
				size = 0x10000;
			}
			fread(ram, size, 1, f);
			fclose(f);
	...
	undo_newest = 0;
	undo[0] = 0;
	undo[UNDO_BUFFER_SIZE-1];

	mutexBP = CreateMutex(nullptr, false, L"6502 Context");
	...
	if (HaveUndoStep()) {
		if (uint32_t stored = undo[undo_newest]) { // 0 means at end of buffer
			...
			for (uint32_t c = 1; c<stored; c++) {
				...
				ram[(uint16_t(change[1])<<8) + change[0]] = change[2];
			}
...
void CPUGoThread()
{
	...
	hThreadCPU = CreateThread(NULL, CPU_EMULATOR_THREAD_STACK, (LPTHREAD_START_ROUTINE)CPUGoThreadRun, nullptr,
							  0, NULL);
}


static void *CPUReverseThreadRun(void *param)
{
	// copy breakpoints to stack so UI can modify original freely
	uint16_t _aBP_PC[MAX_PC_BREAKPOINTS];
	struct sBPCond _aBP_CN[MAX_PC_BREAKPOINTS];
...
void CPUReverseThread()
{
	...
	hThreadCPU = CreateThread(NULL, CPU_EMULATOR_THREAD_STACK,
							  (LPTHREAD_START_ROUTINE)CPUReverseThreadRun, nullptr,
							  0, NULL);
}
...
int Disassemble(uint16_t addr, char *dest, int left, int &chars, bool illegals)
{
	const dismnm *opcodes = a6502_ops;
	int bytes = 1;
	int orig_left = left;
	...
	if (not_valid) {
		...
		switch (mode) {
			...
			case AM_REL:		// 8 ($1234)
				...
				if (label = GetSymbol(arg)) {
					size_t newlen = 0;
					wcstombs_s(&newlen, label8, label, sizeof(label8)-1);
					prtd = sprintf_s(dest, left, aAddrModeLblFmt[mode], mnemonic, label8, arg);
				} else
